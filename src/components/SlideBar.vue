<template>
  <div class="authorinfo">
	...
	<div class="recent_topics">
	   ...
	   <ul>
	      <li v-for="userinfo in topiclimited">   <!--这里使用了计算属性-->
		        <router-link :to="{
					name:'post_content',
					params:{
						id:userinfo.id
...
export default {
  ...
  methods:{
	  getUserInfoData(){ // 这里的name为name:post.author.loginname
		    this.$http.get(`https://cnodejs.org/api/v1/user/${this.$route.params.name}`)// SlideBar和UserInfo的请求相同
			.then((response)=>{
